...
from .settings import PASSTHROUGH_SLUG, ShimModel, PROVIDER_NAME, default_model_slug
...
def catalog_entry(model: ShimModel) -> dict:
    ...
    return {
        ...
        "description": f"{model.display_name} via local BYOK shim.",
...
def codex_config_overrides(catalog_path: Path, default_slug: str, port: int) -> list[str]:
    return [
        ...
        f'model_providers.{PROVIDER_NAME}.name="Codex BYOK Shim"',
